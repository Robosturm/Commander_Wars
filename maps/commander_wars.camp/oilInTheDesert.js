var Constructor = function() { // scriptStart
    ...
    // preconditionend
        if (turn === 1 && player === 0) { // 0 Start Of Turn
            ...
            var dialog2 = GameAnimationFactory.createGameAnimationDialog(qsTr("Capture the the pipe station with the commando in the North-west. The task force in South-east will provide distraction. Keep in mind that there is a Orange Star gaarrison nearby that will surely respond to our attack. Complete the objectives before they have a chance to reinforce the station."), "CO_FORSYTHE", GameEnums.COMood_Normal, "#bc8248"); dialog2.loadBackground(""); // 1 ScriptEventDialogItem
    ...
    // preconditionend
        if (map.getTerrain(16, 6).getBuilding().getOwner() !== null && map.getTerrain(16, 6).getBuilding().getOwner().getPlayerID() === 0 && variable1.readDataBool() === false) {// 0 Building Captured
            ...
            var dialog1 = GameAnimationFactory.createGameAnimationDialog(qsTr("Excellent. Now retreat to the rendezvous coordinate as planed and await further others."), "CO_FORSYTHE", GameEnums.COMood_Happy, "#bc8248"); dialog1.loadBackground(""); // 1 ScriptEventDialogItem
