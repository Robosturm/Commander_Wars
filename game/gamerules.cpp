...
void GameRules::setGatewayHosting(bool newGatewayHosting)
{
    m_gatewayHosting = newGatewayHosting;
}
...
void GameRules::setSpecialDestruction(bool newSpecialDestruction)
{
    m_specialDestruction = newSpecialDestruction;
}
...
void GameRules::createFogVision()
{
    ...
    if (pPlayer != nullptr)
    {
        for (qint32 x = 0; x < width; x++)
        {
            for (qint32 y = 0; y < heigth; y++)
            {
                createFieldFog(x, y, pPlayer, fogOfMistColor, fogOfWarColor);
            }
        }
    }
...
void GameRules::createFieldFog(qint32 x, qint32 y, Player* pPlayer, QColor fogOfMistColor, QColor fogOfWarColor)
{
    ...
    if (pUnit != nullptr)
    {
        ...
        if (visible != GameEnums::VisionType_Clear &&
            visible != GameEnums::VisionType_Mist)
        {
