...
FieldInfo::FieldInfo(Terrain* pTerrain, Unit* pUnit)
{
    ...
    if (pTerrain != nullptr)
    {
        spTerrainInfo pTerrainInfo = spTerrainInfo::create(pTerrain->getMap(), pTerrain, m_pPanel->getWidth() - 80);
    ...
    if (pUnit != nullptr)
    {
        spUnitInfo pUnitInfo = spUnitInfo::create(spUnit(pUnit), m_pPanel->getWidth() - 80);
