                                     ...
                                     bool mirrored)
{
    ...
    CONSOLE_PRINT("RandomMapGenerator::randomMap widht=" + QString::number(width) + " heigth=" + QString::number(heigth) +
                   " seed=" + QString::number(startSeed) + " mirrored=" + QString::number(mirrored), GameConsole::eDEBUG);
    ...
    CONSOLE_PRINT("Creating map with start width: " + QString::number(startWidth) + " start heigth: " + QString::number(startHeigth) +
                   ...
                   " mirror Mode Y: " + QString::number(static_cast<qint32>(mirrorY)), GameConsole::eDEBUG);
...
void RandomMapGenerator::randomMapPlaceTerain(GameMap* pMap, QString terrainID, qint32 width, qint32 heigth, float placeChance, QPoint placeSize, QStringList topTerrainIDs, QList<QVariant> placeChances, GameEnums::RandomMapTerrainType type, QRandomGenerator& randInt)
{
    qint32 maximumTry = 1000;
    if ((width * heigth * placeChance < 1.0f) && (placeChance > 0.0f))
    {
        randomMapPlaceGroup(pMap, randInt.bounded(0, width), randInt.bounded(0, heigth), 1, terrainID, 1, randInt);
    }
    else
    {
        ...
        while (chance > 0 && maximumTry > 0)
        {
            ...
            switch (type)
            {
                ...
                {
                    chance -= randomMapPlaceLine(pMap, randInt.bounded(0, width), randInt.bounded(0, heigth), groupSize, terrainID, randInt);
                    break;
                }
                case GameEnums::RandomMapTerrainType_Group:
                {
                    chance -= randomMapPlaceGroup(pMap, randInt.bounded(0, width), randInt.bounded(0, heigth), groupSize, terrainID, 1, randInt);
                    break;
                }
            }
            maximumTry--;
    ...
    {
        randomMapPlaceOnTop(pMap, terrainID, topTerrainIDs[i], placeChances[i].toFloat(), randInt);
    }
}

qint32 RandomMapGenerator::randomMapPlaceLine(GameMap* pMap, qint32 startX, qint32 startY, qint32 count, QString terrainID, QRandomGenerator& randInt)
{
    CONSOLE_PRINT("RandomMapGenerator::randomMapPlaceLine " + terrainID + " at x: " + QString::number(startX) + " y: " + QString::number(startY) + " and size " + QString::number(count), GameConsole::eDEBUG);
    ...
    while (count > 0 && maximumTry > 0)
    {
        ...
        if (terrainID != pMap->getTerrain(x, y)->getTerrainID() && randomMapTerrainPlaceable(pMap, x, y, terrainID))
        {
            ...
            switch (direction)
            {
                ...
                {
                    if (randomMapAddTerrainPoint(pMap, points, x, y + 1, terrainID, 0))
                    {
                    ...
                    if (randomMapAddTerrainPoint(pMap, points, x + 1, y, terrainID, 0))
                    {
...
qint32 RandomMapGenerator::randomMapPlaceGroup(GameMap* pMap, qint32 startX, qint32 startY, qint32 count, QString terrainID, qint32 terrainRadius, QRandomGenerator& randInt)
{
    CONSOLE_PRINT("RandomMapGenerator::randomMapPlaceGroup " + terrainID + " at x: " + QString::number(startX) + " y: " + QString::number(startY) + " and size " + QString::number(count), GameConsole::eDEBUG);
    ...
    while (count > 0 && maximumTry > 0)
    {
        ...
        if (terrainID != pMap->getTerrain(points[pInd].x(), points[pInd].y())->getTerrainID() &&
            randomMapTerrainPlaceable(pMap, points[pInd].x(), points[pInd].y(), terrainID))
        {
            maximumTry = maximumTries;
            placed++;
            pMap->replaceTerrain(terrainID, points[pInd].x(), points[pInd].y(), false, false);
            randomMapAddTerrainPoint(pMap, points, points[pInd].x() + 1, points[pInd].y(), terrainID, terrainRadius - 1);
            randomMapAddTerrainPoint(pMap, points, points[pInd].x() - 1, points[pInd].y(), terrainID, terrainRadius - 1);
            randomMapAddTerrainPoint(pMap, points, points[pInd].x(), points[pInd].y() + 1, terrainID, terrainRadius - 1);
            randomMapAddTerrainPoint(pMap, points, points[pInd].x(), points[pInd].y() - 1, terrainID, terrainRadius - 1);
            count--;
        }
        points.removeAt(pInd);
...
bool RandomMapGenerator::randomMapTerrainPlaceable(GameMap* pMap, qint32 x, qint32 y, QString terrainID)
{
...
bool RandomMapGenerator::randomMapAddTerrainPoint(GameMap* pMap, QVector<QPoint>& points, qint32 x, qint32 y, QString terrainID, qint32 terrainRadius)
{
    ...
    if (!randomMapTerrainPlaceable(pMap, x, y, terrainID))
    {
        return false;
    }
    ...
    if (terrainRadius > 0)
    {
        randomMapAddTerrainPoint(pMap, points, x + 1, y, terrainID, terrainRadius - 1);
        randomMapAddTerrainPoint(pMap, points, x - 1, y, terrainID, terrainRadius - 1);
        randomMapAddTerrainPoint(pMap, points, x, y + 1, terrainID, terrainRadius - 1);
        randomMapAddTerrainPoint(pMap, points, x, y - 1, terrainID, terrainRadius - 1);
    }
    return true;
}

void RandomMapGenerator::randomMapPlaceOnTop(GameMap* pMap, QString terrainID, QString topId, float chance, QRandomGenerator& randInt)
{
...
void RandomMapGenerator::randomMapCreateRoad(GameMap* pMap, QRandomGenerator& randInt, QVector<QPoint>& playerPositions)
{
    CONSOLE_PRINT("RandomMapGenerator::randomMapCreateRoad", GameConsole::eDEBUG);
...
QVector<QPoint> RandomMapGenerator::randomMapCreateBuildings(GameMap* pMap, qint32 buildings, QRandomGenerator& randInt, const QVector<std::tuple<QString, float>> & buildingDistributions, const QVector<float> & ownedBaseSize,
                                                             float startBaseSize, qint32& progress, qint32 maxProgess)
{
    CONSOLE_PRINT("RandomMapGenerator::randomMapCreateBuildings " + QString::number(buildings) + " and startBaseSize " + QString::number(startBaseSize), GameConsole::eDEBUG);
    ...
    for (qint32 i = 0; i < pMap->getPlayerCount(); i++)
    {
        ...
        for (qint32 i2 = 0; i2 < maxTries; i2++)
        {
            ...
            if (!playerPositions.contains(position) &&
                randomMapIsBuildingPlace(pMap, "HQ", position.x(), position.y()))
            {
                ...
                UnitPathFindingSystem pfs(pMap, pUnit.get());
    ...
    for (qint32 i = 0; i < playerPositions.size(); i++)
    {
        ...
        for (qint32 i2 = 0; i2 < maxTries; i2++)
        {
            ...
            if (randomMapIsBuildingPlace(pMap, "FACTORY", x, y))
            {
                break;
            }
    ...
    for (qint32 i = 0; i < buildingDistributions.size(); i++)
    {
        ...
        randomMapPlaceBuildings(pMap, buildingID, erg.toString(), buildings, playerPositions,
                                ownedBaseSize, buildingChance, startBaseSize, randInt);
    }
    return playerPositions;
}

void RandomMapGenerator::randomMapPlaceBuildings(GameMap* pMap, QString buildingId, QString baseTerrainID, qint32 buildings, const QVector<QPoint> & playerPositions,
                                                 const QVector<float> & ownedBaseSize, float chance, float startBaseSize, QRandomGenerator& randInt)
{
    ...
    CONSOLE_PRINT("RandomMapGenerator::randomMapPlaceBuildings " + buildingId + " with chance " + QString::number(chance) + " placing: " + QString::number(count) + " playerBuldings: " + QString::number(playerBuldings), GameConsole::eDEBUG);
    ...
    for (qint32 i = 0; i < count; i++)
    {
        ...
        for (qint32 i2 = 0; i2 < maxTries; i2++)
        {
            ...
            if (randomMapIsBuildingPlace(pMap, buildingId, x, y))
            {
                break;
            }
            else if (i2 == maxTries - 1)
...
bool RandomMapGenerator::randomMapIsBuildingPlace(GameMap* pMap, QString buildingId, qint32 x, qint32 y)
{
...
QVector<QPoint> RandomMapGenerator::randomMapCreateBuildings(GameMap* pMap, qint32 width, qint32 heigth, qint32 buildings, QRandomGenerator& randInt, const QVector<std::tuple<QString, float>> & buildingDistributions, const QVector<float> & ownedBaseSize,
                                                             float startBaseSize, qint32& progress, qint32 maxProgess, MirrorMode mirrorX, MirrorMode mirrorY)
{
    ...
    for (qint32 i = 0; i < playerCount; ++i)
    {
        ...
        for (qint32 i2 = 0; i2 < maxTries; i2++)
        {
            ...
            if (x < width && y < heigth &&
                randomMapIsBuildingPlace(pMap, "FACTORY", x, y))
            {
                break;
            }
            else if (i2 == maxTries - 1)
    ...
    for (qint32 i = 0; i < buildingDistributions.size(); i++)
    {
        ...
        float buildingChance = std::get<1>(buildingDistributions[i]) / 100.0f;
        erg = pInterpreter->doFunction(RANDOMMAPGENERATORNAME, "get" + buildingID + "BaseTerrainID");
        randomMapPlaceBuildings(pMap, width, heigth, buildingID, erg.toString(), buildings, playerPositions,
                                ownedBaseSize, buildingChance, startBaseSize, randInt, mirrorX, mirrorY);
    }
    return playerPositions;
}

void RandomMapGenerator::randomMapPlaceBuildings(GameMap* pMap, qint32 width, qint32 heigth, QString buildingId, QString baseTerrainID, qint32 buildings, const QVector<QPoint> & playerPositions,
...
QPoint RandomMapGenerator::getMirroredPosition(GameMap* pMap, qint32 index, QPoint position, MirrorMode mirrorX, MirrorMode mirrorY)
{
    QPoint ret = position;
    if (index > 0)
    {
...
qint32 RandomMapGenerator::getMirroredPlayer(GameMap* pMap, qint32 rotation, qint32 mirrorCount, MirrorMode mirrorX, MirrorMode mirrorY)
{
...
void RandomMapGenerator::randomMapPlaceUnits(GameMap* pMap, QVector<std::tuple<QString, float>> units,
                                             qint32 unitCount,
                                             float startBaseUnitSize,
...
{
    CONSOLE_PRINT("RandomMapGenerator::randomMapPlaceUnits", GameConsole::eDEBUG);
    spLoadingScreen pLoadingScreen = LoadingScreen::getInstance();
    if (distributed)
    {
        for (qint32 hqPos = 0; hqPos < playerPositions.size(); ++hqPos)
        {
            ...
            for (qint32 i = 0; i < hqUnits; ++i)
            {
                ...
                randomMapSpawnUnit(pMap, std::get<0>(units[unitIndex]), hqPos, true, playerPositions, randInt, mirrorX, mirrorY);
            }
            for (qint32 i = 0; i < randomSpawnCount; ++i)
            {
                ...
                randomMapSpawnUnit(pMap, std::get<0>(units[unitIndex]), hqPos, false, playerPositions, randInt, mirrorX, mirrorY);
            }
        }
    }
    else
    {
        ...
        for (qint32 i = 0; i < unitCount; ++i)
        {
            ...
            {
                ++hqPos;
                playerIndex = getPlayerForHqPos(pMap, hqPos, mirrorX, mirrorY);
            ...
            {
                randomMapSpawnUnit(pMap, std::get<0>(units[index]), hqPos, true, playerPositions, randInt, mirrorX, mirrorY);
            }
            else
            {
                randomMapSpawnUnit(pMap, std::get<0>(units[index]), hqPos, false, playerPositions, randInt, mirrorX, mirrorY);
            }
...
void RandomMapGenerator::randomMapSpawnUnit(GameMap* pMap, QString unitId, qint32 hqPos, bool nearHq, const QVector<QPoint> & playerPositions, QRandomGenerator& randInt,
                                            MirrorMode mirrorX, MirrorMode mirrorY)
{
    ...
    for (qint32 i2 = 0; i2 < maximumUnitTry; ++i2)
    {
            ...
            pMap->getTerrain(x, y)->getUnit() == nullptr)
        {
            ...
            pMap->spawnUnit(x, y, unitId, pMap->getPlayer(getPlayerForHqPos(pMap, hqPos, mirrorX, mirrorY)), 0);
            break;
        }
    }
}

qint32 RandomMapGenerator::getPlayerForHqPos(GameMap* pMap, qint32 hqPos, MirrorMode mirrorX, MirrorMode mirrorY)
{
    qint32 player = hqPos;
    if (mirrorX != MirrorMode::none || mirrorY != MirrorMode::none)
    {
        ...
        player = getMirroredPlayer(pMap, rotation, hqPos - rotation * teams, mirrorX, mirrorY);
    }
    return player;
}

void RandomMapGenerator::randomMapCreateRoad(GameMap* pMap, qint32 width, qint32 heigth, QRandomGenerator& randInt, QVector<QPoint>& playerPositions, MirrorMode mirrorX, MirrorMode mirrorY)
{
    CONSOLE_PRINT("RandomMapGenerator::randomMapCreateRoad", GameConsole::eDEBUG);
    ...
    CONSOLE_PRINT("connecting allied player hq's with road", GameConsole::eDEBUG);
    for (qint32 i = 0; i < playerCount - 1; ++i)
    {
        ...
        CONSOLE_PRINT("Connecting : " + QString::number(player1) + " with: " + QString::number(player2), GameConsole::eDEBUG);
        connectPositionsWithRoad(pMap, randInt, position, endPoint, mirrorX, mirrorY, mirrorPlacing);
    }
    // connect mirror parts
    CONSOLE_PRINT("connecting enemy player hq's with road", GameConsole::eDEBUG);
...
void RandomMapGenerator::connectPositionsWithRoad(GameMap* pMap, QRandomGenerator& randInt, QPoint position, QPoint endPoint, MirrorMode mirrorX, MirrorMode mirrorY, qint32 mirrorPlacing)
{
