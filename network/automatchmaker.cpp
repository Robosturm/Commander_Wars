...
void AutoMatchMaker::onNewMatchResultData(const QJsonObject & objData)
{
    ...
    pInterpreter->doFunction(m_matchName, "onNewMatchResultData", args);
}
...
void AutoMatchMaker::createNewGame(const QString & players)
{
    ...
    QJSValueList args({pInterpreter->newQObject(this),
                       ...
                       players,
                       pInterpreter->newQObject(&m_mainServer)});
    pInterpreter->doFunction(m_matchName, "onCreateNewGame", args);
    QString saveFile = "savegames/" + m_matchName + QString::number(m_matchCounter) + ".lsav";
...
{
    auto & database = m_mainServer.getDatabase();
    auto changeQuery = database.exec(QString("UPDATE ") + MainServer::SQL_TABLE_MATCH_DATA + m_matchName + " SET " +
...
void AutoMatchMaker::updateMmr(const QString & player1, const QString & player2, qint32 maxEloChange, GameEnums::GameResult gameResultForPlayer1)
{
    ...
    {
        CONSOLE_PRINT("Failed to read mmr's for match rounds " + m_matchName +
                      " for player " + player1 +
...
void AutoMatchMaker::onNewPlayerData(const QJsonObject & objData)
{
    ...
    QJSValueList args({pInterpreter->newQObject(this),
                       ...
                       maxGames,
                       pInterpreter->newQObject(&m_mainServer)});
    pInterpreter->doFunction(m_matchName, "onNewPlayerData", args);
}
...
bool AutoMatchMaker::doNewPlayerData(const QString & player, qint32 minGames, qint32 maxGames, const QString & metaData, qint32 startMmr)
{
    ...
    if (getMmr(player) < 0)
    {
        QString command = QString("INSERT INTO ") + MainServer::SQL_TABLE_MATCH_DATA + m_matchName + " (" +
